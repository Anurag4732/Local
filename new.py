...
engine = sqlalchemy.create_engine(
    'mysql+pymysql://root:test@localhost:3306/company', convert_unicode=True)
...
def Amazon_scrap(engine, pin_):
    ...
    from datetime import datetime
    query = """
    select Platform,Category,id,Url from url_read_sample where Platform = 'amazon'
    """
        ...
        try:
            try:
                try:  # new more(mulli seller)
                    if driver.find_element_by_xpath("//div/div[@class='a-section a-spacing-small a-spacing-top-small']").is_displayed():
                        ...
                        df = df[['Date', 'Time', 'Product_Category', 'Product_platform',
                                 ...
                                 'Product_discount', 'Seller', 'Product_availability', 'Product_Urls', 'asin_code', 'seller_code', 'pin_code']]
                    ...
                    if driver.find_element_by_xpath("//*[contains(text(), 'Available from')]").is_displayed():
                        ...
                        df = df[['Date', 'Time', 'Product_Category', 'Product_platform',
                                 ...
                                 'Product_discount', 'Seller', 'Product_availability', 'Product_Urls', 'asin_code', 'seller_code']]
                        df.to_sql('scrapped_data_8', con=engine,
                                  if_exists='append', index=False)
                ...
                df = pd.DataFrame({'Date': dat, 'Time': cur_time, 'Product_Category': i[2],
                                   ...
                                   'Product_discount': discount, 'Seller': Seller_only,'pin_code' : pin_code,
...
for pin_ in [560034, 110052, 500001, 400052, 700001]:
